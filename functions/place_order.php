...
/* =========================================== */
function save_order_to_db($data, $connection){
  ...
  if($delivery_type != 'delivery_type' && $total_price != 0 && $choosen_payment != 'choosen_payment'){
    ...
    if($insert_query_run){
      ...
      while ($cart_items = pg_fetch_assoc($cart_sql_run)) {
        ...
        $insert_items_query_run = pg_query($connection, $insert_items_query);

        $product_sql = "SELECT * FROM products WHERE id = '$product_id' LIMIT 1";
        $product_sql_run = pg_query($connection, $product_sql);

        $product_data = pg_fetch_assoc($product_sql_run);
        ...
        $update_qty_query_run = pg_query($connection, $update_qty_query);
      }
      $selete_cart_query = "DELETE FROM carts WHERE user_id = '$user_id'";
      $selete_cart_query_run = pg_query($connection, $selete_cart_query);
...
if(isset($_POST['save_address_btn'])){
  ...
  if(isset($_SESSION['auth'])){
    $address_type = pg_escape_string($connection, $_POST['address_type']);
    $name = pg_escape_string($connection, $_POST['name']);
    $phone_number = pg_escape_string($connection, $_POST['phone_number']);
    $complex_name = pg_escape_string($connection, $_POST['complex_name']);
    $address_street = pg_escape_string($connection, $_POST['address_street']);
    $suburb = pg_escape_string($connection, $_POST['suburb']);
    $town = pg_escape_string($connection, $_POST['town']);
    $province = pg_escape_string($connection, $_POST['province']);
    $postal_code = pg_escape_string($connection, $_POST['postal_code']);
    $user_id = $_SESSION['auth_user']['id'];
...
if(isset($_POST['delete_address_btn'])){
  $page = pg_escape_string($connection, $_POST['page']);
  if(isset($_SESSION['auth'])){
    $address_id = pg_escape_string($connection, $_POST['address_id']);
    $user_id = $_SESSION['auth_user']['id'];

    
    $check_existing_address = "SELECT * FROM address_book WHERE id = '$address_id' AND user_id = '$user_id'";
    $check_existing_address_run = pg_query($connection, $check_existing_address);

    if ($check_existing_address_run && pg_num_rows($check_existing_address_run) > 0) {
      // Delete
...
if(isset($_POST['save_card_btn'])){
  $page = pg_escape_string($connection, $_POST['page']);
  if(isset($_SESSION['auth'])){
    $name_on_card = pg_escape_string($connection, $_POST['name_on_card']);
    $card_number = pg_escape_string($connection, $_POST['card_number']);
    $expiry_month = pg_escape_string($connection, $_POST['expiry_month']);
    $expiry_year = pg_escape_string($connection, $_POST['expiry_year']);
    $cvv = pg_escape_string($connection, $_POST['cvv']);
    $user_id = $_SESSION['auth_user']['id'];
...
if(isset($_POST['delete_card_btn'])){
  $page = pg_escape_string($connection, $_POST['page']);
  if(isset($_SESSION['auth'])){
    $card_id = pg_escape_string($connection, $_POST['card_id']);
    $user_id = $_SESSION['auth_user']['id'];

    
    $check_existing_card = "SELECT * FROM card_details WHERE id = '$card_id' AND user_id = '$user_id'";
    $check_existing_card_run = pg_query($connection, $check_existing_card);

    if ($check_existing_card_run && pg_num_rows($check_existing_card_run) > 0) {
      // Delete
